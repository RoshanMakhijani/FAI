...
class CornersProblem(search.SearchProblem):
    ...
    def isVisited(self,corner_num,state):
        self.corners[corner_num]  
            
    def getRemainCorners(self,state):
...
def cornersHeuristic(state, problem):
        ...
        def getPathCost(path):
          ...
          for i in range(len(path)-1):
              cost +=manhattan_dst(path[i],path[i+1])
          return cost
        ...
        walls=problem.walls
        remn_corners=problem.getRemainCorners(state)
        start=state[0]
        paths=getPaths(start,remn_corners)
        min_cost=-1
        for path in paths:
...
#  return len(foodsList)
  position, foodGrid = state
  import util
  foodsList=foodGrid.asList()
  distBetExtFood = 0
  distToClosestFood = None
  for initialFood in foodsList:
    for nextFood in foodsList:
      if (initialFood, nextFood) in problem.heuristicInfo:
        if problem.heuristicInfo[(initialFood, nextFood)] > distBetExtFood:
          distBetExtFood = problem.heuristicInfo[(initialFood, nextFood)]
      else:
        problem.heuristicInfo[(initialFood, nextFood)] = foodDistance(initialFood, nextFood, problem.startingGameState)
  for food in foodsList:
    if (position, food) in problem.heuristicInfo:
      if distToClosestFood == None or distToClosestFood > problem.heuristicInfo[(position, food)]:
        distToClosestFood = problem.heuristicInfo[(position, food)]
    else:
      problem.heuristicInfo[(position, food)] = foodDistance(position, food, problem.startingGameState)
  if distToClosestFood == None:
    distToClosestFood = 0      
  heuristicValue = distBetExtFood + distToClosestFood
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def registerInitialState(self, state):
        self.actions = []
        currState = state
        while(currState.getFood().count() > 0):
            nextPathSegment = self.findPathToClosestDot(currState) # The missing piece
            self.actions += nextPathSegment
            for action in nextPathSegment:
                currState = currState.generateSuccessor(0, action)
        self.actionIndex = 0
